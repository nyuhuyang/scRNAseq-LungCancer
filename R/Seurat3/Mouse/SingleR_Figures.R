...
(load(file = "output/singler_T_Lung_6_2019728.Rda"))
...
singlerDF$singler1main %>% table() %>% prop.table %>% kable() %>% kable_styling()

singlerDF$singler1main = gsub("Endothelial cells|Epithelial cells|Erythrocytes|Fibroblasts",
                              "Other cell types",singlerDF$singler1main)
...
object <- AddMetaColor(object = object, label= "singler1main", colors = singler.colors[1:8])
...
TSNEPlot.1(object, group.by="singler1main",cols = ExtractMetaColor(object),
           ...
           label.size = 4, repel = T,do.return= T,do.print = T,alpha = 0.9,
           title = "Compare cell types in each sample")

TSNEPlot.1(object, group.by="singler1main",cols = ExtractMetaColor(object),
           split.by = "conditions", ncol = 3,
           label = F,pt.size = 1,no.legend = T,label.repel = F,
           label.size = 4, repel = T,do.return= T,do.print = T,alpha = 0.9,
           title = "Compare cell types in each condition")
kable(table(object$singler1main, object$conditions)) %>% kable_styling()

TSNEPlot.1(object, group.by="singler1main",cols = ExtractMetaColor(object),
           split.by = "groups", ncol = 3,
           label = F,pt.size = 1,no.legend = T,label.repel = F,
           label.size = 4, repel = T,do.return= T,do.print = T,alpha = 0.9,
           title = "Compare cell types in D6 vs D9")
kable(table(object$singler1main, object$groups)) %>% kable_styling()

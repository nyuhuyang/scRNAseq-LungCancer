...
args <- "doc/20190529_scRNAseq_info.xlsx"
...
for(i in sample_n){
        ...
        Seurat_list[[i]] <- CreateSeuratObject(Seurat_raw[[i]],
                                                 min.cells = 0,
                                               min.features = 0)
        Seurat_list[[i]]@meta.data$conditions <- df_samples$conditions[i]
        Seurat_list[[i]]@meta.data$groups <- df_samples$groups[i]
        Seurat_list[[i]]@meta.data$tests <- df_samples$tests[i]
        Seurat_list[[i]]@meta.data$tsne <- df_samples$tsne[i]        
